...
//            ps2.setString(3, ctfMId);
        ...
        JSFUtils.addFacesInformationMessage("Last Balance Calculated Successfully");
        return null;
    }
    ...
    public void getCtfMainId(SelectionEvent selectionEvent) {
        ...
        try {
            ...
            ps2 =  conn.prepareStatement("select type_hierarchy from fac_type ft,facility f where ft.fac_type_id = f.fac_type_id and f.fac_code = ?");
...
System.out.println("ctf_main_id=============================================================="+locCtfMainId);
        ...
        JSFUtils.addFacesInformationMessage(" Values Calculated Successfully ");
        return null;
    }
    ...
    public String saveButton() {
        ...
        if (ADFContext.getCurrent().getViewScope().get("cv") == true) {
            ...
            try {
                ...
                ////////////////////////////WORK
                ps =
                    conn.prepareStatement("update CTF_ITEM set OPEN_BAL = ? ,RECEIPTS =  ? ,ISSUES = ? ,ADJUSTMENTS= ? ,ADJ_TYPE_ID= ? ,CLOSING_BAL= ? ,AVG_MNTHLY_CONS= ? ,QTY_REQUIRED= ? ,QTY_RECEIVED= ? ,NEW_VISITS = ? ,CONT_VISITS= ?  where CTF_MAIN_ID= ? and PROD_ID = ? ");
                ...
                for (int i = 0; i < 12; i++) {
                    ...
                    System.out.println("update CTF_ITEM set OPEN_BAL = " + a1 + ",RECEIPTS = " + a2 + ",ISSUES =" + a3 +
                                       ",ADJUSTMENTS=" + a4 + ",ADJ_TYPE_ID=" + a5 + ",CLOSING_BAL=" + a6 +
                                       ",AVG_MNTHLY_CONS=" + a7 + ",QTY_REQUIRED=" + a8 + ",QTY_RECEIVED=" + a9 +
                                       ",NEW_VISITS=" + a10 + ",CONT_VISITS=" + a11 + " where CTF_MAIN_ID=" + ctf +
                                       " and PROD_ID=" + pid);
                    //PreparedStatement ps = conn.prepareStatement("update CTF_ITEM set OPEN_BAL = "+a1+",RECEIPTS = "+a2+",ISSUES ="+a3+",ADJUSTMENTS="+a4+",ADJ_TYPE_ID="+a5+",CLOSING_BAL="+a6+",AVG_MNTHLY_CONS="+a7+",QTY_REQUIRED="+a8+",QTY_RECEIVED="+a9+",NEW_VISITS="+a10+",CONT_VISITS="+a11+" where CTF_MAIN_ID="+ctf+" and PROD_ID="+pid);
                    ps.setInt(1, a1);
                    ps.setInt(2, a2);
                ...
                ps1 =
                    conn.prepareStatement("update CTF_main set CTF_COMMENTS = '" + comm + "' where CTF_MAIN_ID=" + ctf);
